...
> **实现**：`fredica-webui/app/util/subtitleExport.ts`（`secondsToSrtTime()` / `convertToSrt()` / `downloadSrt()`）、`fredica-webui/app/components/bilibili/BilibiliSubtitlePanel.tsx`（导出按钮）

### 1.1 需求概述 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle-bilibili.tsx`、`fredica-webui/app/util/subtitleExport.ts`

在 `material.$materialId.subtitle-bilibili.tsx` 页面（以及主字幕页）为已加载的 Bilibili 字幕提供"导出 SRT"按钮，点击后下载符合 SRT 格式规范的字幕文件。

### 1.2 数据流现状 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../api/routes/BilibiliVideoSubtitleBodyRoute.kt`、`fredica-webui/app/components/bilibili/BilibiliSubtitlePanel.tsx`
...
### 1.3 SRT 格式规范 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/util/subtitleExport.ts`（`secondsToSrtTime()`）
...
### 1.4 实现方案（纯前端） <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/util/subtitleExport.ts`（`convertToSrt()` / `downloadSrt()`）、`fredica-webui/app/components/bilibili/BilibiliSubtitlePanel.tsx`

字幕数据已在前端内存中，**无需新增后端 API**，直接在前端转换并触发浏览器下载。

#### 工具函数（新增） <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/util/subtitleExport.ts`（`SrtSegment` / `secondsToSrtTime()` / `convertToSrt()` / `downloadSrt()`）
...
#### BilibiliSubtitlePanel 改造 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/components/bilibili/BilibiliSubtitlePanel.tsx`（"导出 SRT"按钮）
...
#### 主字幕页集成 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`
...
### 1.5 改动文件清单 <Badge type="tip" text="已完成" />

> **实现**：`subtitleExport.ts`（新建）、`BilibiliSubtitlePanel.tsx`（改造）
...
> **实现**：Kotlin 侧 `shared/src/commonMain/.../asr/`（service + executor + route_ext + model）+ `shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`；Python 侧 `fredica-pyutil/fredica_pyutil_server/routes/audio.py` + `subprocess/transcribe.py`；前端 `fredica-webui/app/routes/material.$materialId.subtitle.tsx` + `app/util/asrConfig.ts`

### 2.0 前置条件：faster-whisper 自动安装 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmMain/.../python/FasterWhisperInstallService.kt`（`ensureInstalled()`）、`shared/src/jvmMain/.../python/PythonUtil.kt`（`installPackage()`）

#### 2.0.1 依赖关系澄清 <Badge type="tip" text="已完成" />

> **实现**：设计说明章节，无独立实现文件。`faster-whisper` 安装见 `FasterWhisperInstallService.kt`
...
#### 2.0.2 自动安装 faster-whisper <Badge type="tip" text="已完成" />

> **实现**：`FasterWhisperInstallService.ensureInstalled()` → `PythonUtil.Py314Embed.installPackage("faster-whisper==1.2.1")`
...
#### 2.0.3 改动文件清单（前置条件部分） <Badge type="tip" text="已完成" />

> **实现**：`PythonUtil.kt`（`installPackage()`）、`FasterWhisperInstallService.kt`（`ensureInstalled()`）、`MaterialWorkflowService.kt`（`startWhisperTranscribe()`）
...
> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`（ASR 方案页面 UI）
...
> **实现**：`shared/src/commonMain/.../apputil/AppUtil.kt`（`Paths.asrOutputDir()`）、Python 侧 `subprocess/transcribe.py`（chunk 文件落盘）
...
> **实现**：`shared/src/commonMain/.../asr/executor/AsrSpawnChunksExecutor.kt`（动态创建 N 个 TRANSCRIBE Task）、`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`（per-chunk 执行 + `tryMergeChunks()`）

#### 2.3.1 完整任务 DAG <Badge type="tip" text="已完成" />

> **实现**：`AsrSpawnChunksExecutor.kt`（SPAWN_CHUNKS 阶段）、`TranscribeExecutor.kt`（TRANSCRIBE × N 阶段 + 合并）
...
#### 2.3.2 两阶段任务创建 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/executor/AsrSpawnChunksExecutor.kt`（阶段 2 动态创建）、`shared/src/commonMain/.../asr/material_workflow_ext/MaterialWorkflowServiceExt.kt`（`startWhisperTranscribe2()` 阶段 1 创建）
...
#### 2.3.3 TRANSCODE_MP4 的必要性判断 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/material_workflow_ext/MaterialWorkflowServiceExt.kt`（`startWhisperTranscribe2()` 内前置检查）
...
#### 2.3.4 各阶段的暂停 / 恢复 / 中止（清理）设计 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../worker/TaskCancelService.kt`、`TaskPauseResumeService.kt`、`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`（per-chunk cancel/pause/resume）
...
> **实现**：`shared/src/commonMain/.../asr/`（service + executor + route_ext + model）、`shared/src/jvmMain/.../asr/executor/`（TranscribeExecutor）

#### 2.4.1 复用的现有 Executor <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmMain/.../worker/executors/ExtractAudioExecutor.kt`
...
#### 2.4.2 新增 Task 类型 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/executor/AsrSpawnChunksExecutor.kt`、`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`
...
#### 2.4.3 TranscodeMp4Service.kt（新建，Kotlin service） <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmMain/.../worker/executors/TranscodeMp4Executor.kt`（转码复用逻辑）
...
#### 2.4.4 AsrSpawnChunksExecutor.kt（新建） <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/executor/AsrSpawnChunksExecutor.kt`
...
#### 2.4.5 TranscribeExecutor 的 ASR chunk 处理 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`（`canSkip()` + `GpuResourceLock` + Python websocketTask 调用）
...
#### 2.4.6 TranscribeExecutor.tryMergeChunks() 合并逻辑 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`（`tryMergeChunks()` — core region 过滤 + SRT 拼接）
...
#### 2.4.7 TranscribeExecutor 的定位 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`
...
#### 2.4.8 ExtractAudioExecutor 与 ffmpeg 设备适配 <Badge type="tip" text="已完成" />
...
#### 2.4.9 MaterialAsrStartRoute.kt（新建，commonMain） <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/material_workflow_ext/MaterialWorkflowServiceExt.kt`（`startWhisperTranscribe2()`）、`MaterialWorkflowRouteExt.kt`（`handleWhisperTranscribe()`）
...
#### 2.4.10 改动文件清单（后端） <Badge type="tip" text="已完成" />
...
> **实现**：`fredica-pyutil/fredica_pyutil_server/routes/audio.py`（`/audio/extract-split-audio-task`、`/audio/transcribe-chunk-task`）、`routes/asr.py`（`/asr/download-model-task`）、`subprocess/transcribe.py`（`FasterWhisperTranscribeTaskEndpoint`）、`subprocess/download_model.py`、`util/ffmpeg_util.py`、`util/device_util.py`
...
> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`、`material.$materialId.subtitle-asr.tsx`、`app/util/asrConfig.ts`

#### 2.6.1 ASR 方案页面 UI <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`（`AsrModelPickerModal`、`AsrSchemeDetail`）
...
#### 2.6.2 进度聚合 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`（`WorkerTaskListRoute` 轮询 + 进度展示）
...
#### 2.6.3 "导出 SRT" 入口 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/util/subtitleExport.ts`（`downloadSrt()`）
...
#### 2.6.4 前端文件清单 <Badge type="tip" text="已完成" />
...
#### 2.7.1 目标与范围 <Badge type="warning" text="未完成" />
...
#### 2.7.2 页面入口与可见性 <Badge type="warning" text="未完成" />
...
#### 入口 1：`app-desktop-home` 统一入口 <Badge type="warning" text="未完成" />
...
#### 入口 2：`local-resources` 页面 <Badge type="warning" text="未完成" />
...
#### 入口 3：`local-resources/asr` 页面 <Badge type="warning" text="未完成" />
...
#### 入口 4：具体结果页 <Badge type="warning" text="未完成" />
...
#### 2.7.3 为什么使用 jsBridge 而不是 Route API <Badge type="warning" text="未完成" />
...
#### 2.7.4 数据模型设计 <Badge type="warning" text="未完成" />
...
#### 2.7.5 Bridge 能力设计 <Badge type="warning" text="未完成" />

建议新增以下 jsBridge 方法：

##### A. `list_all_asr_results` <Badge type="warning" text="未完成" />
...
##### B. `get_asr_result_detail` <Badge type="warning" text="未完成" />
...
##### C. `delete_asr_result` <Badge type="warning" text="未完成" />
...
##### D. `open_asr_result_dir` <Badge type="warning" text="未完成" />
...
##### E. `reimport_asr_result` <Badge type="warning" text="未完成" />
...
#### 2.7.6 Kotlin 侧安全边界 <Badge type="warning" text="未完成" />
...
#### 2.7.7 页面 UI 设计 <Badge type="warning" text="未完成" />

页面拆成三层：

##### A. `local-resources` 资源首页 <Badge type="warning" text="未完成" />
...
##### B. `local-resources/asr` ASR 资源列表页 <Badge type="warning" text="未完成" />
...
##### C. `local-resources/asr/:resultKey` 具体结果页 <Badge type="warning" text="未完成" />
...
#### 2.7.8 前端交互细节 <Badge type="warning" text="未完成" />
...
#### 2.7.9 与现有字幕页的关系 <Badge type="warning" text="未完成" />
...
#### 2.7.10 改动文件清单 <Badge type="warning" text="未完成" />
...
> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`、`fredica-webui/app/util/asrConfig.ts`、`shared/src/commonMain/.../api/routes/MaterialLanguageGuessRoute.kt`

本节记录对 Phase 1.5 已有 ASR 入口的 5 项 UX 与功能改进，均集中在 `material.$materialId.subtitle.tsx` 及相关后端。

#### 2.8.1 模态框高度溢出（Issue 1） <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`（`max-h-48 overflow-y-auto`）
...
#### 2.8.2 torch 未安装时的行为修正（Issue 2） <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/material.$materialId.subtitle.tsx`（`parseCompatJson`、`AsrSchemeDetail`）、`shared/src/commonMain/.../api/routes/MaterialWorkflowRoute.kt`（model 空值校验）
...
#### 2.8.3 LLM 语言猜测（Issue 3） <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../api/routes/MaterialLanguageGuessRoute.kt`、`fredica-webui/app/routes/material.$materialId.subtitle.tsx`（`langGuessState` + `useEffect` 自动猜测）
...
#### 2.8.4 语言检测后自动选择模型（Issue 4） <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/util/asrConfig.ts`（`pickDefaultModel()`）
...
#### 2.8.5 业务逻辑封装到工具函数（Issue 5） <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/util/asrConfig.ts`（`ALL_WHISPER_MODELS`、`WHISPER_LANGUAGES`、`parseCompatJson`、`pickDefaultModel`）
...
#### 2.8.6 改动文件清单 <Badge type="tip" text="已完成" />
...
### 4.1 为什么每个模型+语言组合独立缓存？ <Badge type="info" text="参考" />

不同模型精度差异大，用户可能对同一素材跑多次（先 tiny 预览，再 large-v3 精转）。独立缓存避免重新转录，也允许用户比较不同结果。

### 4.2 为什么最终采用 N×独立 `TRANSCRIBE` Task + `GpuResourceLock`？ <Badge type="info" text="参考" />
...
### 4.3 为什么需要 ASR_SPAWN_CHUNKS 而不在启动 API 里创建所有 Task？ <Badge type="info" text="参考" />
...
### 4.4 SRT 合并时间偏移 <Badge type="info" text="参考" />
...
### 4.5 `language="auto"` 的处理 <Badge type="info" text="参考" />
...
### 4.6 ffmpeg 设备适配 <Badge type="info" text="参考" />
...
### 4.7 为什么 `.done` 文件应统一带 hash / fingerprint？ <Badge type="info" text="参考" />
...
### 5.3 Python 测试范围 <Badge type="info" text="参考" />
...
### 5.4 分层原则 <Badge type="info" text="参考" />
...
### 5.5 取消 / 暂停 / 清理专项验证 <Badge type="info" text="参考" />
...
### A.1 测试夹具（fixtures） <Badge type="info" text="参考" />
...
### A.2 模型不存在（目录为空） <Badge type="info" text="参考" />
...
### A.3 模型文件不完整（模拟下载中断） <Badge type="info" text="参考" />
...
### A.4 OOM 场景（mock） <Badge type="info" text="参考" />
...
### A.5 设备不支持（mock） <Badge type="info" text="参考" />
...
### A.6 网络问题场景（mock download_model_worker） <Badge type="info" text="参考" />
...
### A.7 完整下载后的完整性检查（集成测试） <Badge type="info" text="参考" />
...
### A.8 错误分类边界用例 <Badge type="info" text="参考" />
...
### A.9 关于新增 Python 源文件 <Badge type="info" text="参考" />
...
> **实现**：`shared/src/commonMain/.../worker/GpuResourceLock.kt`（`withGpuLock()`）

> **已完成**：`GpuResourceLock` 已升级为 `Mutex` + `PriorityQueue` 优先级锁（0-20 范围），详见 **§12 优先级 GPU 资源锁**。

### 6.1 问题背景 <Badge type="tip" text="已完成" />

> **实现**：设计说明章节，实现见 `GpuResourceLock.kt`
...
### 6.2 实现：GpuResourceLock 单例 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../worker/GpuResourceLock.kt`（`Semaphore(1)` → 已升级为 `Mutex` + `PriorityQueue`，见 §12）
...
### 6.3 各 Executor 使用方式 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`、`shared/src/jvmMain/.../worker/executors/TranscodeMp4Executor.kt`
...
### 6.4 注意事项 <Badge type="tip" text="已完成" />

> **实现**：设计说明章节，约束体现在 `GpuResourceLock.kt` 和各 Executor 调用处
...
### 6.5 测试 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmTest/.../worker/GpuResourceLockTest.kt`
...
### 7.1 "允许下载模型"选项 <Badge type="warning" text="未完成" />

#### 7.1.1 AppConfig 新增字段 <Badge type="warning" text="未完成" />
...
#### 7.1.2 MaterialAsrStartRoute 逻辑调整 <Badge type="warning" text="未完成" />
...
#### 7.1.3 任务链更新（允许下载时） <Badge type="warning" text="未完成" />
...
### 7.2 模型完整性检查：结合设备分析工具的两阶段加载策略 <Badge type="warning" text="未完成" />

#### 7.2.1 核心设计原则 <Badge type="warning" text="未完成" />
...
#### 7.2.2 为什么不能固定 `compute_type=int8` <Badge type="warning" text="未完成" />
...
#### 7.2.3 统一加载策略设计 <Badge type="warning" text="未完成" />
...
#### 7.2.4 设备分析工具如何接入 <Badge type="warning" text="未完成" />
...
#### 7.2.5 `DownloadWhisperModelExecutor.canSkip` 修改 <Badge type="warning" text="未完成" />
...
##### `.done + hash` 通用模式 <Badge type="warning" text="未完成" />
...
##### 模型检查场景中的 `.done` <Badge type="warning" text="未完成" />
...
##### `canSkip()` 判定升级 <Badge type="warning" text="未完成" />
...
#### 7.2.6 各错误场景与处理 <Badge type="warning" text="未完成" />
...
### 7.3 前端设置页面 <Badge type="warning" text="未完成" />
...
### 8.1 任务链进度合并策略 <Badge type="warning" text="未完成" />
...
### 8.2 下载阶段进度展示 <Badge type="warning" text="未完成" />

这里应当先做**对 faster-whisper / HuggingFace 下载链路的 monkey-patch 设计测试**，再做 UI 展示。

#### 8.2.1 monkey-patch 设计前提 <Badge type="warning" text="未完成" />
...
#### 8.2.2 monkey-patch 测试设计 <Badge type="warning" text="未完成" />
...
#### 8.2.3 运行时进度展示 <Badge type="warning" text="未完成" />
...
### 8.3 转录阶段进度展示 <Badge type="warning" text="未完成" />
...
### 8.4 UI 阶段标签 <Badge type="warning" text="未完成" />
...
### 8.5 已跳过任务的进度处理 <Badge type="warning" text="未完成" />
...
### 8.6 整体 UI 进度条设计 <Badge type="warning" text="未完成" />
...
### 9.1 Phase 1 — 字幕导出（纯前端，无依赖） <Badge type="info" text="参考" />

**最先实现**，完全独立，可并行于其他 Phase。

#### 9.1.1 测试目标 <Badge type="info" text="参考" />

验证 `subtitleExport.ts` 中 `convertToSrt()` 和 `secondsToSrtTime()` 的正确性，覆盖边界值与精度。

#### 9.1.2 测试文件 <Badge type="info" text="参考" />
...
> **实现**：`shared/src/jvmTest/.../python/FasterWhisperInstallServiceTest.kt`、`shared/src/jvmTest/.../db/MaterialWorkflowServiceAsrPayloadTest.kt`

#### 9.1.5.1 测试目标 <Badge type="tip" text="已完成" />

> **实现**：设计说明章节，测试见 §9.1.5.2 和 §9.1.5.3
...
#### 9.1.5.2 测试文件 A：安装服务 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmTest/.../python/FasterWhisperInstallServiceTest.kt`
...
#### 9.1.5.3 测试文件 B：工作流 payload 路径衔接 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmTest/.../db/MaterialWorkflowServiceAsrPayloadTest.kt`
...
#### 9.1.5.4 关键设计说明 <Badge type="tip" text="已完成" />

> **实现**：设计说明章节，约束体现在 `PythonUtil.kt`（`installPackage` Mutex）、`FasterWhisperInstallService.kt`（`@Volatile installed`）
...
### 9.2 Phase 2 — Python 文件锁工具（`file_lock_util.py`） <Badge type="info" text="参考" />

**在开发任何依赖锁的 Executor 之前**完成，因为 Python `transcribe.py` 和 `audio.py` 都需要它。

#### 9.2.1 测试目标 <Badge type="info" text="参考" />

验证 `exclusive_file_lock` 的互斥语义：同一进程内两个线程竞争同一锁文件，后者必须等待前者释放。

#### 9.2.2 测试文件 <Badge type="info" text="参考" />
...
### 9.3 Phase 3 — 模型完整性检查与下载 hook 设计（Python） <Badge type="info" text="参考" />

详细测试用例见 **附录 A**，此处补充 TDD 流程说明。

#### 9.3.1 TDD 流程 <Badge type="info" text="参考" />
...
#### 9.3.2 TDD 顺序建议 <Badge type="info" text="参考" />
...
#### 9.3.3 本阶段最重要的设计约束 <Badge type="info" text="参考" />
...
### 9.4 Phase 4 — AsrSpawnChunksExecutor（Kotlin） <Badge type="info" text="参考" />
...
#### 9.4.1 测试目标 <Badge type="info" text="参考" />
...
#### 9.4.2 测试文件 <Badge type="info" text="参考" />
...
### 9.5 Phase 5 — TranscribeExecutor（ASR chunk 处理，Kotlin） <Badge type="info" text="参考" />

这是 ASR 主链的核心阶段，每个 chunk 一个独立 TRANSCRIBE Task，重点验证 **per-chunk 执行 + GpuResourceLock 串行 + canSkip + 暂停/恢复/取消**。

#### 9.5.1 测试目标 <Badge type="info" text="参考" />
...
#### 9.5.2 测试文件 <Badge type="info" text="参考" />
...
#### 9.5.3 暂停 / 恢复 / 中止（清理）专项约束 <Badge type="info" text="参考" />
...
### 9.6 Phase 6 — TranscribeExecutor.tryMergeChunks()（Kotlin） <Badge type="info" text="参考" />

#### 9.6.1 测试目标 <Badge type="info" text="参考" />
...
#### 9.6.2 测试文件 <Badge type="info" text="参考" />
...
### 9.7 Phase 7 — MaterialAsrStartRoute（Kotlin） <Badge type="info" text="参考" />

#### 9.7.1 测试目标 <Badge type="info" text="参考" />
...
#### 9.7.2 测试文件 <Badge type="info" text="参考" />
...
### 9.8 Phase 8 — DownloadWhisperModelExecutor canSkip 修改 <Badge type="info" text="参考" />

#### 9.8.1 测试目标 <Badge type="info" text="参考" />

- `canSkip` 旧逻辑（目录名检查）改为调用 Python HTTP 接口 → 新测试验证 `canSkip` 在 mock Python 返回 `ready=true/false` 时的行为

#### 9.8.2 测试文件 <Badge type="info" text="参考" />
...
### 9.9 Phase 9 — 前端 ASR 工具函数 <Badge type="info" text="参考" />

#### 9.9.1 测试目标 <Badge type="info" text="参考" />

- `asr.ts` 中 `startAsrTask()` 封装正确（成功 + 错误响应处理）
- `TASK_PHASE_LABEL` 映射完整（所有 task type 有对应中文标签）
- 进度聚合函数：`computeAsrProgress()` 正确计算整体百分比

#### 9.9.2 测试文件 <Badge type="info" text="参考" />
...
### 9.9.3 本地转译结果管理页（jsBridge only） <Badge type="info" text="参考" />
...
#### 测试文件 A：前端页面 <Badge type="info" text="参考" />
...
#### 测试文件 B：前端 util <Badge type="info" text="参考" />
...
#### 测试文件 C：Kotlin JsBridge handler <Badge type="info" text="参考" />
...
#### 推荐 TDD 顺序 <Badge type="info" text="参考" />
...
### 9.10 各 Phase 测试运行命令汇总 <Badge type="info" text="参考" />
...
### 9.11 测试驱动开发节奏 <Badge type="info" text="参考" />
...
> **实现**：`shared/src/commonMain/.../asr/service/AsrConfigService.kt`、`asr/route_ext/AsrConfigRouteExt.kt`、`asr/model/AsrConfigModels.kt`、`fredica-webui/app/routes/app-desktop-setting-faster-whisper-asr-config.tsx`

> 对应需求：服主权限管理（允许下载开关、允许模型列表）+ 桌面设置入口。

### 10.1 功能概述 <Badge type="tip" text="已完成" />

> **实现**：设计说明章节，实现见 §10.2–§10.6
...
### 10.2 数据模型 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../apputil/AppConfig.kt`（ASR 字段）、`shared/src/commonMain/.../asr/model/AsrConfigModels.kt`

#### AppConfig 新增字段 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../apputil/AppConfig.kt`（`asrAllowDownload`、`asrDisallowedModels`、`asrTestAudioPath`、`asrTestWaveCount`）
...
#### ASR 配置响应/请求模型 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/model/AsrConfigModels.kt`（`AsrConfigResponse`、`AsrConfigSaveParam`）
...
### 10.3 服务层：AsrConfigService <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/service/AsrConfigService.kt`（`getAsrConfig()`、`saveAsrConfig()`、`isModelAllowed()`、`isDownloadAllowed()`）
...
### 10.4 API 层 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/route_ext/AsrConfigRouteExt.kt`、`composeApp/src/commonMain/.../appwebview/messages/`（Bridge Handlers）

#### Route Extension（HTTP API，供 Web 模式使用） <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/route_ext/AsrConfigRouteExt.kt`
...
#### Bridge Handlers（桌面端 jsBridge，薄层委托） <Badge type="tip" text="已完成" />

> **实现**：`composeApp/src/commonMain/.../appwebview/messages/`（`GetAsrConfigJsMessageHandler`、`SaveAsrConfigJsMessageHandler`、`SelectAudioFileJsMessageHandler`）
...
### 10.5 权限执行点 <Badge type="tip" text="已完成" />

> **实现**：`MaterialWorkflowRouteExt.kt`（`handleWhisperTranscribe()`）、`TranscribeExecutor.kt`、`fredica-webui/app/util/asrConfig.ts`（`filterDisallowedModels()`）、`material.$materialId.subtitle.tsx`

权限检查通过 `AsrConfigService` 统一执行，分布在以下位置：

#### 后端执行点 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../asr/material_workflow_ext/MaterialWorkflowRouteExt.kt`（`handleWhisperTranscribe()`）、`shared/src/jvmMain/.../asr/executor/TranscribeExecutor.kt`
...
#### 前端执行点 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/util/asrConfig.ts`（`filterDisallowedModels()`）、`fredica-webui/app/routes/material.$materialId.subtitle.tsx`（`AsrModelPickerModal`）
...
### 10.6 前端页面 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/app-desktop-setting.tsx`（入口卡片）、`fredica-webui/app/routes/app-desktop-setting-faster-whisper-asr-config.tsx`（配置页面）

#### 设置页入口 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/app-desktop-setting.tsx`（ASR 配置入口卡片）

**文件**：`fredica-webui/app/routes/app-desktop-setting.tsx`

在 PyTorch 配置卡片之后添加 ASR 配置入口卡片，点击跳转到配置页面。

#### 配置页面 <Badge type="tip" text="已完成" />

> **实现**：`fredica-webui/app/routes/app-desktop-setting-faster-whisper-asr-config.tsx`
...
### 11.1 功能概述 <Badge type="warning" text="未完成" />
...
### 11.2 服务层：AsrModelTestService <Badge type="warning" text="未完成" />
...
### 11.3 数据模型 <Badge type="warning" text="未完成" />
...
### 11.4 Python 端点 <Badge type="warning" text="未完成" />

#### WebSocket 路由 <Badge type="warning" text="未完成" />

**文件**：`desktop_assets/.../routes/asr.py` — 新增 `WS /asr/test-model-task`

#### 子进程 Worker <Badge type="warning" text="未完成" />
...
### 11.5 前端交互 <Badge type="warning" text="未完成" />
...
> **实现**：`shared/src/commonMain/.../worker/GpuResourceLock.kt`（优先级 `Mutex` + `PriorityQueue<WaitEntry>`）、`shared/src/commonMain/.../worker/TaskPriority.kt`

> 对应需求：将 `GpuResourceLock` 从 `Semaphore(1)` 升级为优先级+时间序锁。
>
> 详细优先级策略见 [task-model.md §7.3](../task-model.md#_7-3-优先级策略)。

### 12.1 设计目标 <Badge type="tip" text="已完成" />
...
### 12.2 优先级映射 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../worker/TaskPriority.kt`（所有优先级常量集中定义）
...
### 12.3 实现方案 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/commonMain/.../worker/GpuResourceLock.kt`（`Mutex` + `PriorityQueue<WaitEntry>` + `CancellableContinuation`）
...
### 12.4 优先级传播链 <Badge type="tip" text="已完成" />

> **实现**：`MaterialWorkflowRouteExt.kt` → `MaterialWorkflowServiceExt.kt` → `CommonWorkflowService.kt` → `Task.priority` → `TranscribeExecutor.kt`（`withGpuLock`）
...
#### 修改文件清单 <Badge type="tip" text="已完成" />
...
### 12.5 测试 <Badge type="tip" text="已完成" />

> **实现**：`shared/src/jvmTest/.../worker/GpuResourceLockTest.kt`（7 个测试用例：互斥性、等待状态文本、高优先级抢占、同优先级时间序、取消移除）
...
### 方案二：`download_root` 参数（需改代码） <Badge type="info" text="参考" />
