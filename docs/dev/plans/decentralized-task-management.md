...
| `DOWNLOAD_SUBTITLE` | 下载平台字幕文件（B站 CC/AI 字幕），需登录凭据。无凭据时以 `AWAITING_CREDENTIAL` 错误暂停，用户可选择"跳过"（将任务标记为 completed/skipped）或配置凭据后重试。 | `DOWNLOAD_BILIBILI` | 无 | `CHECK_EXISTING` | <1 min |
...
**数据模型**
- [x] 创建 `task`、`pipeline`、`task_event` 数据表（含 `idempotency_key UNIQUE`、`error_type`、`cache_policy` 字段）
- [x] 实现任务状态机（`pending → claimed → running → completed / failed`）（§5.2）
- [x] `task` 表新增 `is_paused` 字段，支持暂停/恢复语义

**任务执行引擎**
- [x] 实现单节点 Worker 协程池（受 `max_workers` 限制），FIFO + 优先级调度
- [x] 实现 Pipeline DAG 引擎：解析 `depends_on`，前置任务全部 `completed` 后触发下游
- [x] 任务去重：`idempotency_key` 的 `INSERT OR IGNORE` 语义；pipeline 级幂等创建（§4.4）
- [x] 素材导入改为创建 pipeline 而非同步执行
- [x] `TaskCancelService`：运行中任务取消信号注册表（CompletableDeferred）
- [x] `TaskPauseResumeService`：运行中任务暂停/恢复信号注册表（Channel）

**Python 服务 WebSocket 任务框架**
- [x] `util/task_endpoint_util.py`：`TaskEndpoint` + `TaskEndpointInEventLoopThread` 基类（WebSocket 协议：init_param_and_run / cancel / pause / resume / status）
- [x] `PythonUtil.websocketTask()`：Kotlin 端 WebSocket 通道（cancel/pause/resume 信号透传）
- [x] `routes/bilibili_video.py`：WS `/bilibili/video/download-task/{bvid}/{page}`（实时进度推送）
- [x] `app.py` 改为自动扫描 `routes/` 目录动态加载路由

**支持的任务类型（本地基础版，无评分体系）**
- [x] `DOWNLOAD_BILIBILI_VIDEO`（WebSocket 下载，实时进度 0-100%，支持取消/暂停/恢复）
- [x] `DOWNLOAD_BILIBILI_SUBTITLE`（Python HTTP，无凭据时返回 AWAITING_CREDENTIAL）
- [ ] `EXTRACT_AUDIO` → `SPLIT_AUDIO`（旧实现已删，待用 WebSocket 框架重写）
- [ ] `TRANSCRIBE_CHUNK` → `MERGE_TRANSCRIPTION`（旧实现已删，待重写）
- [ ] `AI_ANALYZE`（stub 已删，待重写）

**API（已实现，与原计划略有调整）**
- [x] `POST /api/v1/MaterialRunTaskRoute`（为素材启动单任务流水线）
- [x] `GET /api/v1/WorkerTaskListRoute`（任务列表，分页+状态/素材/分组过滤）
- [x] `POST /api/v1/TaskCancelRoute`（取消单个任务）
- [x] `POST /api/v1/TaskPauseRoute` / `POST /api/v1/TaskResumeRoute`（暂停/恢复单个任务）
- [x] `GET /api/v1/MaterialActiveTasksRoute`（查询素材当前任务快照）
- [x] `POST /api/v1/MaterialDownloadStatusRoute`（批量检查本地下载状态）
- ~~`POST /api/v1/pipelines` · `GET /api/v1/pipelines/{id}` · `GET /api/v1/pipelines` · `DELETE /api/v1/pipelines/{id}`~~（Pipeline 中心路由已删除，由以上 Task 级路由替代）

**WebUI**
- [x] 任务中心页 `tasks._index.tsx`（全局任务列表，支持暂停/恢复/取消，分页，状态/素材/分组过滤）
- [x] 素材库页 `material-library._index.tsx`（含任务状态徽章 + 下载状态展示）

**TDD 要点**

*单元测试：*
- [x] DAG 依赖解析：循环检测（应抛出错误）· 多入度节点（所有前置 `completed` 才触发）· 孤立节点（无依赖立即可认领）
- [x] 状态机迁移：合法路径全覆盖（`pending→claimed→running→completed`）；非法跳转（如 `pending→completed`）被拒绝并报错
- [x] 幂等键去重：相同 key 二次 INSERT 返回原记录 ID，pipeline 级：同素材 + 模板有活跃流水线时不重复创建
- [x] Worker 并行上限：`max_workers=2` 时第 3 个任务等待，前两个之一完成后立即认领
- [x] 优先级调度：`priority=8` 的任务先于 `priority=3` 的被认领（队列中同时存在时）

*集成测试：*
- [x] Pipeline 状态联动：全部子任务 `completed` → pipeline `completed`；任一子任务达 `max_retries` → pipeline `failed`
- [ ] 取消传播：`TaskCancelService.cancel(taskId)` 后，running 任务收到信号；pending 任务 DB 状态变 `cancelled`
- [ ] 端到端冒烟：用 FakeExecutor 模拟完整流水线，最终所有任务状态均为 `completed`，pipeline 状态联动

**里程碑（Phase 1 Done）**

> 所有 TDD 测试绿；本机端到端跑通基础流水线；WebUI 任务列表正确展示进度与状态。
>
> **当前状态（2026-03-06）**：核心引擎与 WebUI 已完成；剩余工作为其余 Executor 重写（ExtractAudio/SplitAudio/TranscribeChunk/Merge/AI）+ 集成测试补充。
...
| Executor 位置 | ~~`commonMain/worker/executors/NetworkTestExecutor.kt`~~（已删除，待用新框架重写） |
