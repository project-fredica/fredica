...
capabilities:
  ...
  download:
    ...
    bilibili:
      ...
      # Desktop 节点留空 = 从 desktop_assets 自动检测
      # Server/NAS 节点填写 pip 安装后的路径，或留空（需已 pip install fredica-pyutil）
      pyutil_path: ""
...
> - **Desktop 节点**：随 `desktop_assets/common/fredica-pyutil/` 自动打包，启动时自动探测，无需额外操作。
> - **Server / NAS 节点**：需手动 `pip install fredica-pyutil` 或在 `worker.yaml` 中指定 `pyutil_path`；未安装则不声明此能力。
...
**下载能力矩阵**（§3.1）
- [ ] `DOWNLOAD_BILIBILI`：bilibili-pyutil，自动检测 `desktop_assets` / 手动 pip install
