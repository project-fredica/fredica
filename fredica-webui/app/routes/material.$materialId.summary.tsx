...
// ─── Idle state: start analysis wizard ───────────────────────────────────────
            ...
            {MODELS.map(m => (
                <label
                    ...
                    className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                        selectedModel === m.id ? 'border-violet-400 bg-violet-50' : 'border-gray-200 hover:border-gray-300 bg-white'
                    }`}
                >
                    <input type="radio" name="llm-model" value={m.id} checked={selectedModel === m.id}
                        onChange={() => setSelectedModel(m.id)} className="accent-violet-600 mt-0.5" />
                    ...
                    <SectionCard title="关键词" icon={<Tags className="w-4 h-4" />}>
                        <div className="flex flex-wrap gap-2">
                            {MOCK_KEYWORDS.map(kw => (
                                <span key={kw} className="text-xs px-2.5 py-1 bg-violet-50 text-violet-700 rounded-full font-medium">
                                    {kw}
                                </span>
                            ))}
                        </div>
